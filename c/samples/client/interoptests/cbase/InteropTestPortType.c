...
xsd__string echoString(void* pStub, xsd__string Value0)
{
	...
	if (AXIS_SUCCESS != pCall->_functions->Initialize(pCall->_object, C_RPC_PROVIDER)) return Ret;
...
xsd__string_Array echoStringArray(void* pStub, xsd__string_Array Value0)
{
	...
	Axis_Array array;
	/* Following will establish the connections with the server too */
	if (AXIS_SUCCESS != pCall->_functions->Initialize(pCall->_object, C_RPC_PROVIDER)) return RetArray;
...
SOAPStruct* echoStruct(void* pStub, SOAPStruct* Value0)
{
	...
	/* Following will establish the connections with the server too */
	if (AXIS_SUCCESS != pCall->_functions->Initialize(pCall->_object, C_RPC_PROVIDER)) return pReturn;
