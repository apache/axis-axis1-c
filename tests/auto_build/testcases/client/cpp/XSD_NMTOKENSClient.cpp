...
{
    char endpoint[256];
    const char* url="http://localhost:80/axis/XSD_NMTOKENSPort";
    ...
    try
    {
        ...
        XSD_NMTOKENSPort* ws = new XSD_NMTOKENSPort(endpoint);
      
        char emptyNMTOKENS[1] = "";
        xsd__NMTOKENS emptyInput = new char[1];
        strcpy (emptyInput, emptyNMTOKENS);
        ...
        char reservedCharactersNMTOKENS[] = "<>&\"\'";
        xsd__NMTOKENS reservedCharactersInput = reservedCharactersNMTOKENS;
        ...
        // Test non-nillable element with XML reserved characters
        char whitespaceNMTOKENS[] = "  \t\r\nsome text \t\r\nmore text \t\r\n";
        xsd__NMTOKENS whitespaceInput = whitespaceNMTOKENS;
        ...
        input = new char[25];
        strcpy (input, simpleNMTOKENS);
        xsd__NMTOKENS nillableResult = ws->asNillableElement(input);
        if (nillableResult)
        {
            if (*nillableResult)
            {
