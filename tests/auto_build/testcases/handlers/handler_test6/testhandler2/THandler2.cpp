...
int THandler2::invoke(void *pvIMsg)
{
	...
	AxisChar* name="tp1";
	AxisChar* exp_value="Set property in handler";

	if(pIMsg->isPastPivot()) {
		/*this is a response*/
	} else {
		/*this is a request*/
		AxisChar* req_value = (AxisChar*)pIMsg->getProperty(name);
		if(!req_value) req_value="unset";
		if(strcmp(exp_value, req_value) != 0) {
