...
public ArrayOfSOAPStruct(){
	...
	private xsd.org.soapinterop.SOAPStruct[] param;
	public xsd.org.soapinterop.SOAPStruct[] getParam(){
		return this.param;	
}
	public void setParam(xsd.org.soapinterop.SOAPStruct[] param){
		this.param = param;	
}
	...
	try{
		...
		while(!((org.apache.axismora.wsdl2ws.java.ParmWriter.tag.getType() == org.xmlpull.v1.XmlPullParser.END_TAG) && 
			...
			xsd.org.soapinterop.SOAPStruct  item =(new xsd.org.soapinterop.SOAPStruct());
			item.desierialize(msgdata);
			obj.add(item);
			 org.apache.axismora.wsdl2ws.java.ParmWriter.tag = msgdata.getTag();
		}
		int length = obj.size();
		xsd.org.soapinterop.SOAPStruct[] arr = new xsd.org.soapinterop.SOAPStruct[length];
		for(int i = 0;i<length;i++)
			arr[i] = (xsd.org.soapinterop.SOAPStruct)obj.get(i);
		 this.param = arr;
	...
	public void serialize(org.apache.axis.encoding.SerializationContext context)throws java.io.IOException{
		...
		for (int i = 0; i < param.length; i++) {
			context.writeString("<item"+i+ "  xsi:type=\"ns4:SOAPStruct\"  xmlns:ns4 = \"http://soapinterop.org/xsd\">");
			if(param[i]!=null){
